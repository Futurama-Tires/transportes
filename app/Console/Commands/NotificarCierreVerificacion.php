...
use App\Notifications\CierreVerificacion;
use App\Models\User;

class NotificarCierreVerificacion extends Command
{
    protected $signature = 'verificacion:notificar-cierre {--dias=7} {--role=administrador}';
    protected $description = 'Notifica vehículos cuyo período de verificación cierra en N días.';
    ...
    public function handle(): int
    {
        ...
        // Mismos criterios que la de “apertura”, solo cambia la fecha: usamos c.vigente_hasta
        $rows = DB::table('vehiculos as v')
            ->join('calendario_verificacion as c', function ($j) use ($target) {
                $j->on('v.estado', '=', 'c.estado')
                  ->whereDate('c.vigente_hasta', '=', $target);
            })
            ->leftJoin('verificaciones as ver', function ($j) {
                // Excluir si ya fue APROBADO/APROBADA dentro de la ventana [desde, hasta]
        ...
        if ($rows->isEmpty()) {
            $this->info("No hay vehículos cuyo período cierre el {$target}.");
            return self::SUCCESS;
        }
        ...
        Notification::send($destinatarios, new CierreVerificacion($payload, $target));

        $this->info("Notificada cierre: {$rows->count()} vehículo(s), fecha {$target}, a {$destinatarios->count()} usuario(s).");
        return self::SUCCESS;
    }
}
