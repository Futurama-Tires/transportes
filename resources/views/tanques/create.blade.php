<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('vehiculos.tanques.store', $vehiculo) }}" novalidate>
                @csrf
                ...
                <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <div class="px-6 py-6">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select id="tipo_combustible" name="tipo_combustible" required
                                    ...
                                    @foreach(['Magna','Diesel','Premium'] as $c)
                                        <option value="{{ $c }}" @selected(old('tipo_combustible')===$c)>{{ $c }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" value="{{ old('capacidad_litros',0) * old('rendimiento_estimado',0) }}"
                                ...
                                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Se calcula automáticamente al guardar.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 px-6 py-4 dark:border-slate-700">
                        ...
                        <button type="submit"
                                class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-700">Guardar</button>
                    </div>
                </div>
            </form>
        </div>
